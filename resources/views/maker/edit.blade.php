<x-base-layout>
    <x-slot name="title">メーカー更新</x-slot>
    <x-slot name="slot">
        <div class="w-full">
            <x-auth-validation-errors class="mb-4" :errors="$errors" />
            <form id="form1" name="form1"
                  class="w-full"
                  action="{{ route('maker.update', ['maker'=>$maker->id]) }}"
                  method="post">
                @csrf
                @method('put')
                <div class="flex flex-col">
                    <div class="px-2 mb-4 w-1/4 sm:w-1/4 lg:w-1/4">
                        ...
                        <x-input id="code" class="w-full" type="number" name="code" :value="old('code', $maker->code)" required autofocus />
                    </div>
                </div>
                <div class="flex flex-col">
                    <div class="px-2 mb-4 w-1/4 sm:w-1/4 lg:w-1/4">
                        <x-label for="name" class="w-full" value="メーカー名" />
                        <x-input id="name" class="w-full" type="text" name="name" :value="old('name', $maker->name)" required />
                    </div>
                </div>
