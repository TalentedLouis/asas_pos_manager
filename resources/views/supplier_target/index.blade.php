<x-base-layout>
    <x-slot name="title">仕入先一覧</x-slot>
    <x-slot name="slot">
        ...
        <table class="table-auto w-full mb-2">
            <thead>
            <tr class="border">
                <th class="py-2 px-1 sm:px-2 lg:px-4 w-1/12 sm:w-1/12 lg:w-1/12 text-right">仕入先コード</th>
                <th class="py-2 px-1 sm:px-2 lg:px-4 w-2/12 sm:w-2/12 lg:w-2/12 text-left">仕入先名</th>
            ...
            @foreach($supplierTargets as $supplierTarget)
                <tr class="border bg-white odd:bg-gray-100">
                    <td class="py-2 px-1 sm:px-2 lg:px-4 w-1/12 sm:w-1/12 lg:w-1/12 text-right">{{ $supplierTarget->code }}</td>
                    <td class="py-2 px-1 sm:px-2 lg:px-4 w-2/12 sm:w-2/12 lg:w-2/12 text-left">{{ $supplierTarget->name }}</td>
                    <td class="py-2 px-1 sm:px-2 lg:px-4 w-9/12 sm:w-9/12 lg:w-9/12 text-left">
                        ...
                        <x-delete :route="route('supplier_target.destroy', ['supplier_target'=>$supplierTarget->id])">
